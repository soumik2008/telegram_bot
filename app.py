...
# Vercel के लिए Flask app बनाएं
@app.route('/')
def home():
    return jsonify({
        "status": "running",
        "message": "NoTmeowL Free Fire Bot API",
        "endpoints": ["/join", "/status", "/commands"],
...
def status():
    return jsonify({
        "status": "online",
        "bot_running": online_writer is not None,
...
def commands_list():
    return jsonify({
        "commands": [
            ...
            "/em [uid] [emote] - Send emote",
            "/nm [team_code] [uid] [emote] - Join team and emote",
            "/like [uid] - Send likes",
            "/info [uid] - Get player info",
            "/clan [id] - Get clan info",
            "/visit [uid] - Visit profile",
            "/3 - Create 3-player squad",
            "/5 - Create 5-player squad",
            "/6 - Create 6-player squad",
            "/solo - Leave squad",
            "/boost - Speed boost"
        ]
    })
...
# Helper Functions
def is_admin(uid):
    ADMIN_UID = "4231218210"  # Change this to your admin UID
...
async def SEndMsG(H , message , Uid , chat_id , key , iv):
    from xC4 import xSEndMsgsQ, xSEndMsg
    TypE = await cHTypE(H)
    if TypE == 'Squid': 
        ...
        try:
            ...
            while True:
                ...
                if data2.hex().startswith('0500') and len(data2.hex()) > 1000:
                    try:
                        from xC4 import DeCode_PackEt, GeTSQDaTa
                        from xC4 import AutH_Chat
...
[FFB300]NoTmeowL BOT"""
                        ...
                        # SQUAD COMMANDS (enhanced)
                            ...
                            try:
                                ...
                                from xC4 import OpEnSq, cHSq, SEnd_InV
...
async def perform_emote(team_code: str, uids: list, emote_id: int):
    ...
    try:
        # Join the squad 😮
        from xC4 import GenJoinSquadsPacket, Emote_k
